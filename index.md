...
Lead engineer on **TOOL** (Trustless Orderflow Operations Layer) — an extensive fork of go-ethereum that turns Ethereum's monolithic 12-second block-building pipeline into a decentralised, TEE-attested P2P network producing 1-second pre-confirmations on mainnet. The product addresses the centralisation and privacy gaps of today's MEV-Boost supply chain: instead of a few opaque builders gatekeeping orderflow, any operator can compete privately while still committing to common, hardware-verified execution logic.
